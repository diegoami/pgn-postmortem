...
- **Scope:** a new page, `quiz.html`, linked from the top of the index.
  - **Who the player is:** `build_site` gains `player` and `aliases` parameters (today it has
    none; the names exist only when reading). `Collection.build_site` passes on the names its
    collection was read with, and the `site` command passes its `--player`/`--alias`. Names match
    exactly as `Collection.read` matches them: letter case and surrounding spaces are ignored.
    - **No player** (neither `player` nor `aliases` given): there is no quiz page and no link to it,
      because there are no "own" moves.
    - **A game where both White and Black match** the player's names: both sides' moments count as
      the player's.
  - **Which questions:** every critical moment in the player's games that was played by the player,
    using the same rules as the articles (F-6's critical moments and outcome swings; F-5's
    presumed results don't affect moments). The opponents' moments stay in their articles only.
  - **Order:** by winning chances lost (the exact, unrounded value), worst first. Ties are broken by
    the game's position in the index (date order), then by the article's file name, then by move
    order.
  - **One line per question:** its rank, the game (date and opponent), the move played (e.g.
    ...
    The page shows no diagram and no answer, so it stays small on a phone. Each line carries two
    `data-` attributes, the same names F-8 uses: the game's id and the move (e.g. `30b`). F-8's
    "only these" list of `data-` attributes is extended by exactly these two on the quiz page.
  - **A player with no critical moments of their own** gets a quiz page that says so, instead of an
    empty list.
...
- **Done when:** the gates pass, including:
  1. **Python tests on fixtures:**
     ...
     - they are in worst-first order, with the whole tie rule tested (equal points, then index
       order, then file name, then move order);
     - the player's names match regardless of letter case and surrounding spaces, and a game where
       both sides are the player counts both sides;
     - each line links to an existing `moment-N` anchor of that move's question (the link checker
       covers the page), and each line's `data-` game and move equal those on the answer it links to;
     - without a player, there is no quiz page and the index has no link to it; a player with no
       own moments gets the "none" page;
     - a rebuild without a player removes a `quiz.html` the builder wrote, and never one it didn't
       write;
     - F-8's `data-` attribute test covers the quiz page, with exactly the two new attributes;
...
- **Open questions:** decided by the owner on 2026-09-25, each against a recommended default:
  ...
  - **Proposed with this shaping, confirmed by the owner's merge:**
    ...
    - the tie rule (exact points, then index order, then file name, then move order);
    - the line's contents and its two `data-` attributes;
    - no quiz page when the site has no player, and a "none" page for a player without own moments;
    - `player` and `aliases` on `build_site`, matched like `Collection.read`, and both sides counted
      when both match;
...
- **Scope:** two plain links to lichess, with no script and nothing loaded by the page itself.
  ...
  - **"Open this game on lichess"** in each article's infobox. It links to
    ...
    **without the check and mate signs** (`+`, `#`). These are optional in SAN, and a `+` in the path
    ...
    - **a game that starts from a set-up position**, decided by the board the game starts from
      (owner, 2026-09-25, on review 023's finding 1: a `FEN` header that spells out the standard
      start still gets a game link): lichess accepts a `[FEN …]`
...
- **Out of scope:**
  ...
  - **Note for later** (owner, 2026-09-25, on review 023's finding 2): a game of a chess variant (a
    `Variant` header other than standard) gets a standard-chess game link, as the whole site treats
    games as standard chess. None of the 148 analyzed games of the owner's book has a `Variant`
    header (checked by the orchestrator on 2026-09-25); the rest of the archive is not checked yet.
