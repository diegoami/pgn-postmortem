...
def analyze_files(
    ...
    """Analyze every file in pgn_paths into out_dir/<same name>, running
    `workers` single-threaded Stockfish processes side by side (0 = one per
    CPU). Threads are enough to drive them: the Python side only waits on
    engine output. Returns how many games were written."""
    out_dir.mkdir(parents=True, exist_ok=True)
    workers = min(workers or os.cpu_count() or 1, len(pgn_paths)) or 1
    ...
    def run(pgn_path: Path) -> bool:
        source_game = read_single_game(pgn_path)
        if source_game is None:
            return False
        engine = engines.get()
        try:
            out_game = analyze_game(engine, limit, source_game, pv_plies, thresholds)
        finally:
            engines.put(engine)
        (out_dir / pgn_path.name).write_text(format_game(out_game), encoding="utf-8")
        return True
    ...
    try:
        with ThreadPoolExecutor(max_workers=workers) as pool:
            futures = {pool.submit(run, path): path for path in pgn_paths}
            for done, future in enumerate(as_completed(futures), start=1):
                written += future.result()
                elapsed = time.monotonic() - started
                print(f"  [{done}/{len(pgn_paths)}] {futures[future].name}  ({elapsed:.0f}s)", flush=True)
    finally:
...
def main() -> None:
    ...
    parser.add_argument(
        "--workers", type=int, default=0, help="parallel engine processes (default: one per CPU)"
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    print(f"Analyzing {len(todo)} game(s)...")
    written = analyze_files(todo, games_out_dir, limit, args.pv_length, thresholds, args.workers)
    print(f"Wrote {written} game(s) to {games_out_dir}")
