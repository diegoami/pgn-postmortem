...
| 7 | F-10, "A link in critical position to a pop up link, where you can start stockfish and analyze the current position" | F-10's block in `ROADMAP.md` | F-11, F-1.3, F-1.4, F-2, F-3, F-4, F-7 | Claude Code (the owner's standing choice of 2026-09-24, applied 2026-09-25) | none: Claude mode has no design stage (`CLAUDE.md`); the plan is F-10's block in `ROADMAP.md` | small | a fresh-context session (`CLAUDE.md`) |
| 8 | F-12, "ok can we update the pgn postmortem demo to reflect the chess games collection ?" | F-12's block in `ROADMAP.md` | F-11, F-1.3, F-1.4, F-2, F-3, F-4, F-7 | Claude Code (the owner's standing choice of 2026-09-24, applied 2026-09-26) | none: Claude mode has no design stage (`CLAUDE.md`); the plan is F-12's block in `ROADMAP.md` | small | a fresh-context session (`CLAUDE.md`) |
| after F-12 (F-11 is parked) | F-1.3 of F-1, "first publishing a pip / python library that creates a wikipedia / epub from a pgn collection" | F-1.3 in F-1's block | F-1.4, F-2, F-3 | chosen by the owner | written when the iteration starts (OpenCode mode) | large | the assignment table (OpenCode) or a fresh-context session (Claude) |
