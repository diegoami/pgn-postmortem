...
| 5 | F-1.3 of F-1, "first publishing a pip / python library that creates a wikipedia / epub from a pgn collection" | F-1.3 in F-1's block | F-1.4, F-2, F-3 | chosen by the owner | written when the iteration starts (OpenCode mode) | large | the assignment table (OpenCode) or a fresh-context session (Claude) |
