...
| 5 | F-8, "A local history reminding what games have you been watching and ideally the spoilers you have looked" | F-8's block in `ROADMAP.md` | F-9, F-10, F-11, F-1.3, F-1.4, F-2, F-3, F-4, F-7 | Claude Code (owner, 2026-09-24) | none: Claude mode has no design stage (`CLAUDE.md`); the plan is F-8's block in `ROADMAP.md` | medium | a fresh-context session (`CLAUDE.md`) |
