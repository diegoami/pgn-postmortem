...
|---|---|---|---|---|---|---|---|
| 1 | F-1, "first publishing a pip / python library that creates a wikipedia / epub from a pgn collection" | F-1's block in `ROADMAP.md`, written when the request is shaped | F-2, F-3 | chosen by the owner when the iteration starts | written when the request is shaped (OpenCode mode) | per request | the assignment table (OpenCode) or a fresh-context session (Claude) |
| 2+ | the first unblocked roadmap request, in the owner's order | that request's block in `ROADMAP.md` | the other requests | chosen by the owner | written when the request is shaped (OpenCode mode) | per request | as above |

F-1 is expected to be larger than one iteration; shaping it splits it before
any work starts (`ROADMAP.md`, *Size it to one iteration*).
