...
|---|---|---|---|---|---|---|---|
| 1 | F-1.1 of F-1, "first publishing a pip / python library that creates a wikipedia / epub from a pgn collection" | F-1.1 in F-1's block in `ROADMAP.md` | F-1.2 to F-1.4, F-2, F-3 | OpenCode (owner, 2026-09-24) | `design/001-f1-1-read-and-analyze.md`, written by the implementer when the iteration starts and agreed before any code | medium | the assignment table (OpenCode) or a fresh-context session (Claude) |
