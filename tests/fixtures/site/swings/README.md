...
bands are the default 35/65: **White winning** at 65% or more, **Black
winning** at 35% or less, **level** in between.
...
| `not-swings.pgn` | 6... Bg4 | 51.8% → 62.6% (level → level) | 10.8 | its own, 6... O-O | a 10–20-point loss inside one band (item 4) |
| | 7. h3 | 62.6% → 75.1% (level → White winning) | −12.5 (a gain) | 6... Bg4's refutation, 7. b4 | a band change in favour of the side that moved (item 2) |
| | 8. Nbd2 | 71.5% → 63.5% (White winning → level) | 8.0 | 8. g4 (as an analysis with a lower inaccuracy threshold would store it) | a band change costing less than 10 points (item 3) |
...
| `edges.pgn` | 7. Re1 | 51.8% → 36.5% (level → level at 35/65) | 15.3 | its own, 7. Bb3 | Black winning, and a swing, with the lower edge set to exactly `win_percent(-150)` (item 7) |
| | 8... Nh5 | 51.8% → 63.5% (level → level at 35/65) | 11.6 | its own, 8... Ba7 | White winning, and a swing, with the upper edge set to exactly `win_percent(150)` (item 7) |

Every other move costs nothing (its eval equals the previous one, or it gains).
The item numbers are F-6's done-when items in `ROADMAP.md`.
