...
- **the canonical source:** `scripts/*.py` for the Markdown pipeline's behaviour,
  ...
  command in `tests/test_site.py`) and `examples/site/analyzed/**` (the Pages
  demo's analysis, ROADMAP F-12, written once with Stockfish by
  `.venv/bin/python -m pgn_postmortem analyze examples/daily_games --depth 22 --out examples/site/analyzed`;
  to change the engine or the depth, delete the directory and analyze again);
  change the code and regenerate. `data/openings/*.tsv` is a copy of the
...
- **paths to normally ignore:** `.venv/` (the local environment),
  ...
  whole); `examples/docs/**`, `examples/analyzed_games/**` and
  `examples/site/analyzed/**` (generated, see above; open one file when a
  check points at it); any workspace `.cache/`
...
- **the gates table:**
  ...
  deploys the demo site (ROADMAP F-12) as one Pages artifact: at the root, the
  library's site of the five classic games, built by the package installed
  from the checkout from the committed analysis in `examples/site/analyzed/`
  (the workflow runs no Stockfish), and at `/markdown/`, the Markdown demo
  built with Jekyll from `examples/docs/`, its `baseurl` the Pages base path
  plus `/markdown`. It runs on a push to `main` that touches
  `examples/docs/**`, `examples/site/**`, `pgn_postmortem/**`,
  `pyproject.toml`, `requirements.txt` or the workflow, and on
  `workflow_dispatch`; its deploy job runs only on `main`, so a dispatch on a
  branch builds and deploys nothing. It depends on network and GitHub
  availability, so a red run is re-run once. A second red run is
  investigated: only a failure caused by this repository (the workflow, the
  library's code in `pgn_postmortem/**`, the package manifests
  `pyproject.toml` and `requirements.txt`, or the content of `examples/docs/`
  or `examples/site/analyzed/`) is a defect, handled by the defect path in
